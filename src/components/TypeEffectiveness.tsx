...
export function TypeEffectiveness({ types }: { types: string[] }) {
  ...
  const Section = ({ label, items, mult }: { label: string; items: string[]; mult?: string }) => (
    items.length > 0 ? (
      <div>
        <h4 className="mb-2 text-sm font-semibold text-muted-foreground">{label} {mult && <span className="text-xs">{mult}</span>}</h4>
        <div className="flex flex-wrap gap-1.5">{items.map((t) => <TypeBadge key={t} type={t} size="sm" />)}</div>
      </div>
    ) : null
  );
  return (

...
function PokemonDetailPage() {
  ...
  if (error || !data) return <div className="mx-auto max-w-2xl px-4 py-10 text-center"><p>{t.error}</p><Link to="/" className="mt-4 inline-block text-primary underline">{t.backHome}</Link></div>;
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-6">
      ...
      <div className={cn("relative overflow-hidden rounded-3xl p-6 md:p-8 pokeball-bg", typeBgClass(primary))}>
        ...
        <div className="relative grid items-center gap-6 md:grid-cols-[auto_1fr]">
          {(p.artwork_url || p.sprite_url) && (
            <img src={p.artwork_url ?? p.sprite_url ?? ""} alt={name} className="mx-auto h-48 w-48 object-contain drop-shadow-xl md:h-64 md:w-64" />
          )}
          <div className="text-center text-white md:text-start">
            ...
            <div className="mt-3 flex flex-wrap justify-center gap-2 md:justify-start">
              {p.types.map((tt) => <TypeBadge key={tt} type={tt} size="lg" />)}
            </div>
            <div className="mt-4 grid grid-cols-2 gap-2 text-sm">
              <div className="rounded-lg bg-black/20 px-3 py-2"><div className="opacity-80">{t.pokemon.height}</div><div className="font-semibold">{(p.height / 10).toFixed(1)} {t.pokemon.meters}</div></div>
              <div className="rounded-lg bg-black/20 px-3 py-2"><div className="opacity-80">{t.pokemon.weight}</div><div className="font-semibold">{(p.weight / 10).toFixed(1)} {t.pokemon.kilograms}</div></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mt-6 grid gap-6 md:grid-cols-2">
        ...
        <section className="rounded-2xl border border-border bg-card p-5">
          ...
          <ul className="space-y-1.5">
            {p.abilities.map((a) => (
              <li key={a} className="rounded-md bg-muted px-3 py-1.5 text-sm">{a.replace(/-/g, " ")}</li>
            ))}
          </ul>
        </section>
      </div>
      ...
      <section className="mt-6 rounded-2xl border border-border bg-card p-5">
        ...
        <EvolutionChain evolutions={data.evolutions} chain={data.chainPokemon.length > 0 ? data.chainPokemon : [p]} />
      </section>

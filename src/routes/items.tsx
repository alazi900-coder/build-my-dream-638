  ...
  const filtered = useMemo(() => {
    return all.filter((i) => {
      ...
      if (q) {
        ...
        if (!name.toLowerCase().includes(needle) && !i.name_en.toLowerCase().includes(needle))
          return false;
      }
      return true;
    });
    ...
    <div className="mx-auto max-w-6xl px-4 py-6">
      <div className="mb-6 text-center">
        <h1 className="text-3xl font-bold md:text-4xl">
          {lang === "ar" ? "أدوات البوكيمون" : "Pokémon Items"}
        </h1>
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        <button
          onClick={() => setCat("")}
          className={`rounded-full border px-3 py-1 text-xs font-medium ${cat === "" ? "bg-primary text-primary-foreground border-transparent" : "border-border bg-card text-muted-foreground"}`}
        >
          {lang === "ar" ? "الكل" : "All"}
        </button>
        {categories.map((c) => (
          <button
            key={c}
            onClick={() => setCat(c)}
            className={`rounded-full border px-3 py-1 text-xs font-medium ${cat === c ? "bg-primary text-primary-foreground border-transparent" : "border-border bg-card text-muted-foreground"}`}
          >
            {catLabel(c)}
          </button>
        ))}
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
        {filtered.map((i) => (
          <ItemCard key={i.id} item={i} catLabel={catLabel} />
        ))}
      </div>
    </div>
  );
